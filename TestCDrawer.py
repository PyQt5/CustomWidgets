...
class Window(QWidget):
    ...
    def doOpenTop(self):
        if not hasattr(self, 'topDrawer'):
            self.topDrawer = CDrawer(
                self, stretch=0.5, direction=CDrawer.TOP, styleSheet='background:white;')
        self.topDrawer.show()

    def doOpenLeft(self):
        if not hasattr(self, 'leftDrawer'):
            self.leftDrawer = CDrawer(
                self, direction=CDrawer.LEFT, styleSheet='background:white;')
        self.leftDrawer.show()

    def doOpenRight(self):
        if not hasattr(self, 'rightDrawer'):
            self.rightDrawer = CDrawer(self, styleSheet='background:white;')
    ...
    def doOpenBottom(self):
        if not hasattr(self, 'bottomDrawer'):
            self.bottomDrawer = CDrawer(
                self, direction=CDrawer.BOTTOM, styleSheet='background:white;')
